...
def analisar_tendencias(log_filepath: str, db: Session):
    ...
    try:
        ...
        # 1. Puxa os últimos N sorteios reais (excluindo 9999 de simulação) para calcular frequência e atraso
        ultimos_sorteios = db.query(models.ResultadoLotofacil).filter(models.ResultadoLotofacil.concurso != 9999).order_by(models.ResultadoLotofacil.concurso.desc()).limit(janela).all()
        ...
        historico_completo = db.query(models.ResultadoLotofacil).filter(models.ResultadoLotofacil.concurso != 9999).order_by(models.ResultadoLotofacil.concurso.desc()).all()
